...
void ofApp::setup() {
  ...
  int image_width = static_cast<int>(image_.getWidth()) / 6;
  int image_height = static_cast<int>(image_.getHeight()) / 6;
  image_.resize(image_width, image_height);

  // Resize the canvas to be a square fitting the shortest side of the image
  int window_size = image_width > image_height ? image_height : image_width;
